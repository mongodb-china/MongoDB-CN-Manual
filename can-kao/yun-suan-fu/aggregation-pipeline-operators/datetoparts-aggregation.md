...
* [定义](datetoparts-aggregation.md#definition)
* [行为](datetoparts-aggregation.md#behavior)
* [例子](datetoparts-aggregation.md#example)
...
| 字段 | 必选/可选 | 描述 |
| :--- | :--- | :--- |
| year | 必选 | _在版本3.6中更改。_ 返回部分的输入日期。`<dateExpression>`可以是解析为日期、时间戳或ObjectID的任何表达式。有关表达式的更多信息，请参见表达式。 |
| timezone | 可选 | 用于格式化日期的时区。默认情况下， `$dateToParts`使用UTC。 `<timezone>`可以是任何表达式，该表达式的值可以是: 1. 一个奥尔森时区标识符，例如`"Europe/London"`或`"America/New_York"`， 2. UTC偏移量，格式为： a. `+/-[hh]:[mm]`，例如`"+04:45"` b. `+/-[hh][mm]`，例如`"-0530"` c. `+/-[hh]`例如`"+03"` 有关表达式的更多信息，请参见 表达式。 |
