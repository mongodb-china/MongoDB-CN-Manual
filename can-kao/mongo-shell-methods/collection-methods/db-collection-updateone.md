...
* [定义](db-collection-updateone.md#definition)
* [语法](db-collection-updateone.md#syntax)
* [访问控制](db-collection-updateone.md#access-control)
* [行为](db-collection-updateone.md#behaviors)
* [例子](db-collection-updateone.md#examples)
...
| `upsert` | boolean | 可选的。当`true`，updateOne\(\)时： 1. 如果没有文档匹配`filter`，则创建一个新文档。有关详细信息，请参阅upsert 行为。  2. 更新匹配`filter`的文档。  要避免多次 upsert，请确保`filter`字段为唯一索引。  默认为`false`。 |
