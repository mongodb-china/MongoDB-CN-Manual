...
更新的文档_仅_包含替换文档中的`_id`字段和该字段。这样，这些字段 `ratings`和`reorder`不再存在于更新的文档中，因为这些字段不在替换文档中。
...
如果`multi`设置为`true`，则该 `db.collection.update()`方法将更新_所有_符合`<query>`条件的文档。该`multi`更新操作可以与其他的读/写操作交错。
